...
class FaceDetectionApp {
    ...
    async playSessionVideo(session) {
        ...
        if (!session.video_filename || session.video_filename === 'null') {
            console.warn('⚠️ No video file');
            if (videoLoading) videoLoading.style.display = 'none';
            if (noVideoOverlay) noVideoOverlay.style.display = 'flex';
            return;
        }

        // Tạo video element mới để tránh cached events
    ...
    `;
        ...
        freshVideo.addEventListener('loadedmetadata', () => {
            ...
            if (videoLoading) videoLoading.style.display = 'none';
            freshVideo.style.display = 'block';
            this.displayVideoInfo(session, freshVideo);
        }, { once: true });
        ...
        freshVideo.addEventListener('error', (e) => {
            console.error('❌ Video error:', freshVideo.error);
            if (videoLoading) videoLoading.style.display = 'none';
            if (noVideoOverlay) {
                noVideoOverlay.style.display = 'flex';
                noVideoOverlay.innerHTML = `
                <div class="empty-icon">❌</div>
                <h4>Lỗi video</h4>
                <p>${freshVideo.error?.message || 'Không thể phát video'}</p>
            `;
            }
        }, { once: true });
    ...
    showVideoError(message, videoUrl = '') {
        ...
        if (noVideoOverlay) {
            ...
            <div class="debug-info">
                ...
                <div class="url-preview">${videoUrl.substring(0, 100)}...</div>
            </div>
            ` : ''}
            ...
            <div class="error-actions">
                ...
                ${videoUrl ? `
                <button onclick="window.open('${videoUrl}', '_blank')" class="btn btn-sm btn-info">
                    🌐 Mở trong tab mới
                </button>
                
                <button onclick="window.faceDetectionApp?.downloadVideo('${videoUrl}')" class="btn btn-sm btn-warning">
                    ⬇️ Tải xuống
                </button>
                ` : ''}
            </div>
            ...
            <div class="tech-tips">
                <small><strong>Mẹo khắc phục:</strong></small>
                <ul>
                    <li>Video có thể bị chặn bởi chính sách CORS của Cloudinary</li>
                    <li>Thử mở trong tab mới để kiểm tra</li>
